...
def sales_report_text(sku: int, start: str, end: str) -> str:
    ...
    if allowed and int(sku) not in allowed:
        ...
        now = dt.datetime.now().strftime("%d.%м.%Y %H:%M")
        return (
...
# ─────────────────────────────────────────────────────────────────────────────
# Регистрация алиасов для совместимости с импортами из других частей проекта.
_mod = _sys.modules[__name__]
for _name in (
    "modules_sales.status_report",
    "modules_sales.shipments_status",
    "modules_sales.shipments_status_report",
    "modules_sales.shipments_report",
):
